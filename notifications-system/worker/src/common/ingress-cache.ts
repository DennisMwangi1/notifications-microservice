...
const TENANT_CACHE_TTL_SECONDS = parseInt(process.env.TENANT_API_CACHE_TTL_SECONDS || '900', 10);
const IDEMPOTENCY_TTL_HOURS = parseInt(process.env.IDEMPOTENCY_TTL_HOURS || '24', 10);
 ...
 * @returns redis key string
 */
export function getIdempotencyCacheKey(tenantId: string, idempotencyKey: string): string {
 ...
 */
export async function getCachedTenantIdentity(apiKey: string): Promise<CachedTenantIdentity | null> {
 ...
 */
export async function cacheTenantIdentity(apiKey: string, tenant: CachedTenantIdentity): Promise<void> {
    await redis.set(
 ...
 * @param apiKey tenant API key
 */
export async function invalidateTenantIdentityCache(apiKey: string): Promise<void> {
...
export async function getCachedIdempotencyEntry(
    ...
    return readJsonValue<CachedIdempotencyEntry>(getIdempotencyCacheKey(tenantId, idempotencyKey));
}
...
export async function reserveIdempotencyEntry(
    ...
    const entry: CachedIdempotencyEntry = {
        ...
        payloadHash,
        eventType,
        expiresAt: new Date(Date.now() + getIdempotencyTtlSeconds() * 1000).toISOString(),
    };
 ...
 * @param idempotencyKey idempotency key from request
 */
export async function clearIdempotencyEntry(tenantId: string, idempotencyKey: string): Promise<void> {
